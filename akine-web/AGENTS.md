...
This file defines the frontend-specific rules for `akine-web/akine-web`.
...
### Required agent output for visual tasks

Before implementing any visual change, explicitly state:
...
If this explicit validation is not provided, the frontend task is considered incomplete. A visual change cannot be marked as done without verifying compliance with `AKINE_UI_RULES.md`.
